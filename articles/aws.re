...
よく「DNSを変更したけど浸透には時間がかかる」のように言われますが、これは正確に言えば「TTLに指定された時間が過ぎるまではフルリゾルバに古いリソースレコードのキャッシュが残っているため、ネームサーバにある新しいリソースレコードの情報を取りに行かず、キャッシュの古いリソースレコードが使われてしまう」ということです。人によって異なるフルリゾルバを使っているため、たとえばサイトリニューアルでAレコードを書き換えたときに、Web制作会社のA社は新しいサイトが見られるがクライアントのB社ではまだ古いサイトが表示される、といった現象が起こります。

理屈さえ分かれば、「浸透」という実態の分からない言葉を信じてただ闇雲に待つ必要はありません。変更前のTTLを把握しておけば、最大でどれだけ待てば新しいリソースレコードの情報に切り替わるのかは分かります。事前にTTLを短くしておくことで反映までにかかる時間を短くすることもできます。いつまでに切り替わるのか正確に分かっていれば、逆にその時間までに切り替わらなかったときに「何かがおかしいぞ」ときちんと気付くことができます。

逆に当面は書き換える予定がないリソースレコードについては、TTLを長くしておけばフルリゾルバが都度ネームサーバまで聞きに来なくてよいのでキャッシュヒット率が上がりますし、万が一ネームサーバが死んでしまったときもしばらくはキャッシュでしのげるので障害の影響範囲を狭めることができます。どうしてもキャッシュを今すぐ消したい！という場合は、もし情シスが自社のフルリゾルバを管理しているのであれば「フルリゾルバのキャッシュをクリアして！」と頼めばキャッシュがなくなって、改めてネームサーバへリソースレコードを問い合わせに行ってくれます。
