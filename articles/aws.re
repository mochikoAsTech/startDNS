...
「DNSをはじめよう」の続編である「AWSをはじめよう」（@<img>{startAWS}）は、AWSの環境で実際に手を動かしてサーバを立て、自分のドメイン名でウェブサイトを作って公開するところまでがっつりやるハンズオン形式の本です。BOOTH@<fn>{howToBuy}で購入できますので、本著を読み終わった後に「折角ドメイン名買ったし、サイトを作るところまでやってみたい！」と思ったら続けてチャレンジしてみてください。
