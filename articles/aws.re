...
今回はAWSの中でもRoute53（ルートフィフティースリー）というDNSのサービスを使用します。クラウドといえばGoogleのGoogle Cloud PlatformやMicrosoftのAzure（アジュール）、最近シェアを伸ばしてきたAlibaba Cloud、さくらインターネットのクラウドやGMOクラウドなどAWS以外にもたくさんあります。ですが現状のシェアトップはダントツでAWS@<fn>{cloudShare}です。AWSなら何か困って検索したときに出てくる情報も多いので、今回はAWSのRoute53で自分のドメイン名のゾーンを作成します。
...
//footnote[awsFree][無料利用枠の範囲が決まっており、何をどれだけ使っても無料という訳ではありませんので注意してください。たとえばこの後利用するRoute53というDNSのサービスは、1つのドメイン名につき毎月50セントかかります。詳細は@<href>{https://aws.amazon.com/jp/free/}を確認してください]
...
電話による確認（@<img>{phoneCall}）で電話番号（例：090-1234-5678）とセキュリティチェックを入力したら、携帯電話を手元に用意した状態で「すぐに連絡を受ける」を押します。

//image[phoneCall][電話番号とセキュリティチェックを入力したら「すぐに連絡を受ける」を押す][scale=0.8]{
//}
...
Route53というネームサーバの中に自分のドメイン名のゾーンを作りたいので、ホストゾーンの画面（@<img>{hostedZones}）が表示されたら「Create Hosted Zone」を押してください。
...
このサイトでは自分のドメイン名のネームサーバが何になっているのか、今の状態を確認できます。では「nslookup(dig)テスト【DNSサーバ接続確認】」のページ（@<img>{digStartDnsFunByCman}）で「ホスト名(FQDN)を指定してください」という欄に自分のドメイン名を入力（@<table>{digByCman}）して、オプションで「NS：正式なネームサーバ@<fn>{nameServer}」を選択したら「dig実行」を押してください。
...
//footnote[abTestAgain][ABテストの影響でログインするお名前IDによってドメインNaviの見た目が異なるかもしれません。見た目に関わらず管理画面でやりたいことは同じで「自分が買ったドメイン名のネームサーバを、デフォルトのお名前.comからRoute53に変更したい」だけです]

//image[continueContract][ドメインNavi ドメイン契約更新][scale=0.8]{
//}

繰り返しになりますが「ドメイン契約更新」の画面が表示されるのは、ドメインNaviがログインするたびに「来年の更新をしませんか？」と聞いてくる仕様だからです。先ほど買ったドメイン名の設定変更がしたいので、上部のメニューで左から2つ目の「ドメイン」をクリックしてください。
...
TTLに300と書かれているので、最大で300秒＝5分待てばキャッシュの期限が切れてネームサーバはRoute53に切り替わるはずです。再び「nslookup(dig)テスト【DNSサーバ接続確認】」で「ホスト名(FQDN)を指定してください」という欄に自分のドメイン名を入力（@<img>{digStartDnsFunByCman2}）して、オプションで「NS：正式なネームサーバ」を選択したら「dig実行」を押してください。

//image[digStartDnsFunByCman2][再び自分のドメイン名のネームサーバを調べる][scale=0.8]{
//}
...
正解はBです。今行った作業はドメイン名のネームサーバを、デフォルト設定のお名前.comからAWSのRoute53というネームサーバに変更しただけです。レジストラ（ドメイン名を購入するお店）をお名前.comからRoute53に変更する「レジストラ移管」を行ったわけではないので、1年後に更新確認の連絡をしてくるのもお名前.comです。
...
//image[startAWS][AWSをはじめよう][scale=0.6]{
//}
