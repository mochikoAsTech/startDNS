...
というドメイン名があったとき、次の図（@<img>{startDNS3}）のようにexample（2箇所にある）やcoやjpやcomといった個々の名前空間@<fn>{nameSpace}の領域を「ドメイン」と呼びます。@<fn>{domain}それぞれのドメインを「.（ドット）」でつないで一意に識別する「example.co.jp」や「example.com」といった名前がドメイン名@<fn>{fqdn}です。@<fn>{dot}
