...
ちょっとここで言葉の整理をしましょう。先ほど、購入のために値段を確認した「example.co.jp」は「ドメイン」でしょうか？それとも「ドメイン名」なのでしょうか？@<fn>{differences}結論から言うと、「example.co.jp」はドメイン名です。
...
というドメイン名があったとき、次の図（@<img>{startDNS2}）のようにexample（2箇所にある）やcoやjpやcomといった個々の名前空間@<fn>{nameSpace}の領域を「ドメイン」と呼びます。@<fn>{domain}それぞれのドメインを「.（ドット）」でつないで一意に識別する「example.co.jp」や「example.com」といった名前がドメイン名@<fn>{fqdn}です。@<fn>{dot}
...
//image[startDNS2][個々の名前空間がドメインで、それぞれのドメインを識別するための一意な名前がドメイン名][scale=0.8]{
//}
...
という住所と大変よく似ています。次の図（@<img>{startDNS3}）のように、先ほどのexampleやstartdnsと同様、中央区や南区と呼ばれるエリアは国内の複数箇所に存在していますが、都道府県からの住所を言えば一意に識別できます。

//image[startDNS3][exampleと同じように中央区は複数箇所にあり、一意に識別するために住所がある][scale=0.8]{
//}
...
フィッシング詐欺に限らず、著名な社名やサービス名をドメイン名に含めることで誤認によってサイトに集客する、あるいは今後使われそうなドメイン名を先に登録して高値での転売を持ちかける、といった不正行為に対しては、ドメイン名紛争処理方針@<fn>{drp}という仕組みでも対応が可能です。正当な権利者が申し立てを行い、DRPの紛争処理機関に認められれば、そのドメイン名の登録を取り消したり、権利者に移転したりできます。
...
ここまでドメイン名を「買う」と表記してきましたが、ドメイン名は本当に「買う」ものなのでしょうか？ちょっと立ち止まって考えてみましょう。たとえば花屋さんで一輪のガーベラを買ったら、そのガーベラはあなたの物です。あなたの部屋に飾ってもいいし、他の人にあげてもいいし、瓶詰めのハーバリウムにしてメルカリで売っても構いません。ですが先ほどお名前.comで「買った」ドメイン名は、花と同じようにあなたのものなのでしょうか？これからずっといつまでも使えるのでしょうか？

実はドメインは「買う」ものではなく、登録申請を出して「登録する」ものです。お名前.comのサイトでも「国内シェアNo.1のドメイン登録サービス」「今すぐドメイン登録！」のように、しっかり「登録」と書かれています。
