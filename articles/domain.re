...
お名前.com@<fn>{onamaeURL}でexample.co.jpを調べてみると、1年で3,660円@<fn>{money}でした。（@<img>{onamae3660yen}）しかも残念ながら売り切れなのか×がついて「登録済みドメインのため、お申込みいただけません。」となっています。@<fn>{cannotbuy}
...
そもそもドメイン名は「example.co.jp」や「yahoo.com」のように.（ドット）で区切られています。このとき、いちばん右側のjpやcomをTLD（トップレベルドメイン）と呼びます。@<fn>{tdl}そしてこのTLDは、1つにつき必ず1つのレジストリ（登録管理組織）@<fn>{registryOperator}によって管理されています。
...
たとえばexample.co.jpのTLDはjpですが、このjpというTLDは、日本の「株式会社日本レジストリサービス」（通称JPRS）がレジストリです。JPRSという社名を見聞きしたことはありますか？一時期は車内広告を出したりしていたのでもしかしたら見たことがあるかもしれません。
...
たとえばexample.jpのようにjpで終わるドメイン名なら、先ほどのJPRSが唯一の卸元です。example.co.jpが欲しいときも、example.ne.jpが欲しいときも、example.jpが欲しいときもすべてJPRSがレジストラに卸して（あるいはレジストラからさらにリセラに卸して）そこで私たちが買うのです。
...
最初に「ドメイン」で検索したとき、example.co.jpはYahoo!ドメインでは「取扱いなし」で買えなかったですよね。これはYahoo!ドメインが「うちでは.co.jpを扱わない」という判断をしたからです。

以上がレジストリ、レジストラ、リセラの関係でした。

== 「ドメイン」なのか「ドメイン名」なのか？

ところで、ちょっとここで言葉の整理をしましょう。先ほど、購入のために各社の値段を比較した「example.co.jp」は「ドメイン」でしょうか？それとも「ドメイン名」なのでしょうか？@<fn>{differences}結論から言うと、「example.co.jp」はドメイン名です。
...
このように「株式会社イグザンプル」がexample.co.jpというドメイン名を登録するとき、その時点で使用する予定がなくても@<code>{example.jp}と@<code>{example.com}を一緒に確保しておく、というやり方はよく見かけます。
