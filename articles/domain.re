 ...
 * example.jp
 * example.com
というドメイン名があったとき、次の図（@<img>{startDNS2}）のようにexample（2箇所にある）やcoやjpやcomといった個々の名前空間@<fn>{nameSpace}の領域を「ドメイン」と呼びます。@<fn>{domain}それぞれのドメインを一意に識別するための「example.co.jp」や「example.com」といった名前がドメイン名です。

//image[startDNS2][個々の名前空間がドメインで、一意に識別するための名前がドメイン名][scale=0.8]{
//}
