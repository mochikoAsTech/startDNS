...
//footnote[buyDomain][ところでドメイン名って「買う」ものなの？という疑問については後述します。取りあえず先へ進みましょう]
...
ここでは仮に「筆者が株式会社イグザンプルという会社の広報担当になったので@<code>{example.co.jp}というドメイン名を買おうとしている」という設定であちこちのお店を見てみます。

=== お名前.com

お名前.com@<fn>{onamaeURL}で@<code>{example.co.jp}を調べてみると、1年で3,660円@<fn>{money}でした。（@<img>{onamae3660yen}）しかも残念ながら売り切れなのか×がついて「登録済みドメインのため、お申込みいただけません。」となっています。@<fn>{cannotbuy}
...
続いてNTTPCコミュニケーションズの名づけてねっと@<fn>{naduketeURL}で、@<code>{example.co.jp}の値段を見てみましょう。@<code>{example.co.jp}は属性JP1年プランというプラン名で7,200円でした。（@<img>{nadukete7200yen}）
...
Yahoo!ドメイン@<fn>{domainsYahooURL}でも@<code>{example.co.jp}の値段を調べてみましょう。おや？どうしたのでしょう？プルダウンにco.jpがありません。（@<img>{yahooDomain}）
...
「取得できるドメインの種類」（@<img>{doNotSellCoJp}）を確認したところ、Yahoo!ドメインではどうやら〇〇.co.jpというドメイン名は取り扱いがないため@<code>{example.co.jp}は買えないようです。
...
6社の価格を確認しましたが、値段だけで比べるとお名前.comで買うのがよさそうです。（@<table>{domainFee}）
...
しかしこうして金額を並べてみると色々な疑問が湧いてきませんか？一番高い名づけてねっとの7,200円は、一番安いお名前.comの3,660円と比較すると倍近い金額です。なぜこんなに金額に差があるのでしょう？それになぜYahoo!ドメインだけ@<code>{example.co.jp}を取り扱っていなかったのでしょう？
...
正解はBです。ドメイン名はどこのお店で買っても品質に差はありません。プロ向けに業務用のドメイン名屋さんがあるわけではないので、Web制作会社でもよくお名前.comやムームードメインでドメイン名を購入しています。ロボット掃除機のルンバはどこで買っても同じルンバですが、ヤマダ電機とヨドバシカメラで値段は異なりますよね。ルンバと同じようにドメイン名もどこで買っても同じドメイン名ですがお店によって値段が違うのです。
...
//footnote[muumuu][正確にはムームードメインはjpドメイン名においてのみレジストラです。それ以外のTLDはレジストラ（eNomとお名前.com）から仕入れてリセラとして販売しています]
...
迷ったら取りあえずお名前.comを使っておけば間違いはありません。何しろ取り扱うドメイン名は550種類以上で、累積登録実績2,000万件@<fn>{2000domains}という国内最大のレジストラです。国内だけでなく世界中のレジストラを対象にした新gTLD@<fn>{gTld}の保有数ランキング（@<img>{ntldStats}）@<fn>{tldStats}でもトップ5に入るほどの規模なのです。

//footnote[2000domains][2019年3月15日に累積ドメイン登録実績が2,000万件を突破し、国内のドメイン登録シェアは87％以上だそうです。@<href>{https://www.onamae.com/campaign/20MM/}]
//footnote[gTld][generic Top Level Domainの略。gTLDの一覧は@<href>{https://www.nic.ad.jp/ja/dom/types.html}を参照]
//footnote[tldStats][@<href>{https://ntldstats.com}]
...
今回のように@<code>{example.co.jp}というドメイン名が欲しい場合は、リセラのYahoo!ドメインからでもレジストラのお名前.comからでも購入できて、実はどちらから買っても卸元は同じということです。
...
そもそもドメイン名は「@<code>{example.co.jp}」や「@<code>{yahoo.com}」のように.（ドット）で区切られています。このとき、いちばん右側のjpやcomをTLD（トップレベルドメイン）と呼びます。@<fn>{tdl}そしてこのTLDは、1つにつき必ず1つのレジストリ（登録管理組織）@<fn>{registryOperator}によって管理されています。

//footnote[registryOperator][「レジストリ」という言葉は、ドメイン名の割当や登録といった管理業務を行う「レジストリオペレーター」を指す場合と、そのレジストリオペレーターが登録情報を蓄積している「レジストリデータベース」を指す場合があります。本著では前者を指してレジストリと呼びます]
...
=== 〈トラブル〉Mackerelのアラート誤報はioドメイン名が原因

2017年9月、株式会社はてなが提供しているサーバ監視サービスのMackerelで、ioドメイン名の不調によってアラート誤報を出してしまった、というトラブル（@<img>{mackerel}）がありました。これはioドメイン名のネームサーバが、Mackrelのmackerel.ioというドメイン名について名前解決のリクエストを受けてもきちんと応答できなかったことで、監視対象のサーバがMackerelシステムに通信できなくなり、Mackerelシステムが「監視対象のサーバから連絡が来ない！死んだのか！」と判断して誤発報してしまった、という障害でした。

//image[mackerel][Mackerelの死活監視アラートの誤報を報告するブログ][scale=0.8]{
//}

このトラブルは信用できるレジストリのTLDを選ぶ以外に回避策がありません。絶対に落とせないサイトやメールを消失させたくないメールアドレスなどで、.ioのようにトラブルを起こしたことのあるTLDを採用するのはあまりお勧めしません。ドメイン名を買うときは、このTLDはどこのレジストリが管理しているのだろう？過去に障害は発生していないか？などを事前に確認しておきましょう。たとえばjpドメイン名のJPRSやtokyoドメイン名のGMOインターネットなら、レジストリとしての実績があるため安心です。
...
たとえば@<code>{example.jp}のようにjpで終わるドメイン名なら、先ほどのJPRSが唯一の卸元です。@<code>{example.co.jp}が欲しいときも、@<code>{example.ne.jp}が欲しいときも、@<code>{example.jp}が欲しいときもすべてJPRSがレジストラに卸して（あるいはレジストラからさらにリセラに卸して）そこで私たちが買うのです。

ちなみに卸元のレジストリから、一般消費者がドメイン名を直接買うことはできません。必ずレジストラもしくはリセラを通して購入することになります。
...
AさんとBさんが同時に@<code>{example.co.jp}を買ってしまい、二人とも使おうとしてどちらのサイトを表示したらいいのか大混乱！のようなことが起こらないようにするため、つまりドメイン名の一意性を保つためにレジストリがそのTLDを一元管理しています。

前述のioドメイン名のように、レジストリがそのTLDをきちんと管理してくれないと大変なことになるので、誰でも彼でもレジストリになれる訳ではなく、レジストリになるにはものすごい大金と政府の推薦などが必要なのだそうです。（ブランドTLDはそうした条件が緩和されているため取りやすいようです）

===[column] 【コラム】他のドメイン名と比べてjpドメイン名が高い理由

レジストリはそのTLDの唯一の卸元ですので、もちろん卸値もレジストリが決められます。新興の.workや.xyzなどは卸値が安いのでお名前.comでも1円～30円という破格ですが、jpドメイン名は元々の卸値が高いので2,340円@<fn>{jpPrice}と少し高めの価格で売られています。

一度、GMOインターネットの熊谷社長が「.jpは他国のccTLD@<fn>{ccTld}に比べて高すぎる。独占企業だからと高い値段で売るのではなく国際的な水準まで値下げすべきだ」とJPRSに公開質問@<fn>{kumagai}を投げかけたこともあるほどです。

しかし値段の分だけあって、JPRSは管理体制もしっかりしているので前述のioドメイン名のような障害が起きるリスクは低いといえます。ひとたび障害が起きれば対応する人件費もかかりますし、jpのドメイン名が少々高くても安心代と思えば安いのかも知れません。

高いといえば1年で250,000円かかる.richというドメイン名もあります。もしあなたがリッチなことをアピールしたければ、「自分の名前.rich」のようなドメイン名を買ってみてはいかがですか？もちろんお名前.comで購入できますよ！

===[/column]

//footnote[jpPrice][お名前.comにおける2019年9月時点の値段]
//footnote[ccTld][Country Code Top Level Domainの頭文字を取ってccTLD。国ごとに日本は.jp、フランスは.fr、アメリカは.usのように割り当てられています]
//footnote[kumagai][@<href>{https://www.kumagai.com/?eid=718}、@<href>{https://www.kumagai.com/?eid=732}]

== ICANNがレジストリを決める

TLDが生まれてくる源泉となるレジストリは神のごとき存在で責任重大です。そんな重要なレジストリをどの会社に任せるのか、一体誰が決めるのでしょう？

「君が.jpのレジストリね！」とJPRSを選んだのは誰かというと、ICANN（アイキャン）という組織です。ICANNとはインターネットのIPアドレスやドメイン名などの資源を全世界的に調整・管理する非営利法人です。ここがルートネームサーバを管理しているので、全ドメイン名の本当の生産元と言ってもいいかもしれません。@<fn>{rootServer}

//footnote[rootServer][ルートネームサーバについては@<chapref>{whatIsDns}で後述します]
...
最初に「ドメイン」で検索したとき、@<code>{example.co.jp}はYahoo!ドメインでは「取扱いなし」で買えなかったですよね。これはYahoo!ドメインが「うちでは.co.jpを扱わない」という判断をしたからです。

以上がレジストリ、レジストラ、リセラの関係でした。

== 「ドメイン」なのか「ドメイン名」なのか？

ところで、ちょっとここで言葉の整理をしましょう。先ほど、購入のために各社の値段を比較した「@<code>{example.co.jp}」は「ドメイン」でしょうか？それとも「ドメイン名」なのでしょうか？@<fn>{differences}結論から言うと、「@<code>{example.co.jp}」はドメイン名です。

//footnote[differences][初版を書いたときは2つの表記の違いが分かっておらず、ドメインもドメイン名もごっちゃに書いていました。書くことによって「分かったつもりになっていた自分」が次々見つかるので、これからもまだまだ「DNSなにも分からない」の谷が来るんだろうなと思います]
...
では実際に、お名前.comでドメイン名を買ってみましょう。「お名前.com」で検索してトップページ（@<img>{onamaeTopPage}）を開きます。
...
好きな文字@<fn>{underBar}を入力して、検索ボタンをクリックします。（@<img>{selectDomain}）筆者は「startdns.〇〇」というドメイン名が欲しいので、startdnsと入力しました。

//footnote[underBar][ドメイン名には_（アンダーバー）は使えませんのでご注意ください。英数字や-（ハイフン）は使えます]

//image[selectDomain][好きな文字を入力して検索][scale=0.8]{
//}

すると次の画面で「これは買えるよ、これは売り切れだよ」と教えてくれます。「全て表示」から「取得可能」に切り替えると、現時点で購入可能なドメイン名だけに絞り込まれます。気に入るものが見つかるまで「もっと見る▼」を押してどんどん表示してみましょう。

//image[domainWhoCanBought][取得可能なドメイン名だけを表示して選ぼう][scale=0.8]{
//}

今回はちゃんとしたサービスで使う訳ではなく、ドメイン名やDNSの仕組みを学ぶことが目的なので1円～100円程度の安いドメイン名で構いません。それから詳細は後述しますが.jpは持ち主の名前がWhoisで出てしまうため、本名をインターネットで公表したくない人にはお勧めしません。

また記載されいてるのは有効期間1年のドメイン名の料金ですので、いま買うと1年後に更新タイミングがやってきます。前述のとおり2年目以降はぐっと値段が上がったりしますので、TLDを決めたら更新料金@<fn>{onamaePrice}も確認しておきましょう。購入するドメイン名が決まったらチェックを入れてください。

//footnote[onamaePrice][@<href>{https://www.onamae.com/service/d-price/}]

選択肢がいくつかある中で筆者は@<code>{startdns.fun}を買うことに決めて、.funの下にあるチェックボックスにチェックを入れました。（@<img>{buyFun}）「1ドメイン選択中」と表示されていることを確認したら、「お申し込みへ進む」をクリックします。

//image[buyFun][欲しいドメイン名にチェックを入れて「お申し込みへ進む」][scale=0.8]{
//}

次に表示されるお申込内容の確認画面（@<img>{registOrder}）でとても大切な作業があります。ここでドメイン名の値段を再度確認するとともに、「登録年数/プラン/オプションなど」の欄に@<b>{「Whois情報公開代行」と表示されていることを必ず確認}してください。Whois情報公開代行オプションはドメイン名を買うタイミングで申し込めば無料ですが、後から頼もうとすると980円もかかってしまいます。Whois情報公開代行がどういうものなのかは後述します。

//image[registOrder][「Whois情報公開代行」と表示されていることを確認][scale=0.8]{
//}

なお画面下の方で、まるでセットのポテトのように「ご一緒にサーバはいかがですか？」と聞かれますが、今回はサーバの契約は不要ですのでデフォルトの「利用しない」のままで構いません。右側の「初めてご利用の方」に自分のメールアドレスとパスワードを入れたら「次へ」を押してください。ここで@<b>{入力したパスワードはこの後すぐに使用しますのでしっかりメモしておいて}ください。（@<table>{onamaeData}）
...
//table[onamaeData][お名前.comに登録した情報]{
項目	例	あなたが登録した情報　　　　　　　
------------------------------------
...
次は支払方法の選択画面（@<img>{payment}）が表示されます。右側の「お申込み内容」に、選択したドメイン名の値段およびWhois情報公開代行（0円）が表示されていることを確認してください。「GMOのでんきセット限定特典」は今回は不要ですので、「全額キャッシュバック」のチェックは外しておきましょう。お申込み内容に誤りがなければクレジットカード情報を記入@<fn>{creditCard}して右側にある緑色の「申込む」を押します。
...
ヤマト運輸や佐川急便といった宅配業者の不在通知などを装って、SMS@<fn>{sms}に書かれたURLから偽サイトに誘導し、個人情報を盗み取る「スミッシング@<fn>{smishing}」というサイバー攻撃@<fn>{cyber}があります。たとえば佐川急便では@<code>{sagawa-exp.co.jp}というドメイン名を使用していますが、偽サイトは@<code>{sagawa-umo.com}@<fn>{umo}のようなドメイン名です。（@<img>{startDNS1}）

//image[startDNS1][佐川急便の偽サイトを開こうとするとブラウザが警告してくれる][scale=0.8]{
//}

こうしたフィッシング詐欺や、あたかも公式かのように誤認させるサイトによる被害を防ぐ方法の1つに、類似のドメイン名を予め確保しておく、というものがあります。たとえば任天堂株式会社の国内向けオフィシャルサイトで使用しているドメイン名は@<code>{nintendo.co.jp}ですが、任天堂では@<code>{nintendo.jp}というドメイン名も登録しています。

このように「株式会社イグザンプル」が@<code>{example.co.jp}というドメイン名を登録するとき、その時点で使用する予定がなくても@<code>{example.jp}と@<code>{example.com}を一緒に確保しておく、というやり方はよく見かけます。

ただTLDはたくさんの種類があるため、@<code>{example.jp}と@<code>{example.com}は確保したけど@<code>{example.inc}は他人に取られてもいいの？@<code>{example.site}や@<code>{example.net}は放っておいていいの？というように、いったいどこまで確保すべきなのか、あるいは@<code>{example.jp}と@<code>{example.com}すらも本当に確保しておく意味があるのか、は迷うところかもしれません。ドメイン名は登録した分だけお金がかかります。類似のドメイン名すべてを確保しておくのは現実的ではないので、事業の規模や、フィッシング詐欺の対象となるようなサービスなのか否かを考えて決めるのがよいと思います。

フィッシング詐欺に限らず、著名な社名やサービス名をドメイン名に含めることで誤認によってサイトに集客する、あるいは今後使われそうなドメイン名を先に登録して高値での転売を持ちかける、といった不正行為に対しては、ドメイン名紛争処理方針（DRP）@<fn>{drp}という仕組みでも対応が可能です。正当な権利者が申し立てを行い、DRPの紛争処理機関に認められれば、そのドメイン名の登録を取り消したり権利者に移転したりできます。
...
ドメイン名の購入が完了したら、お名前.comのトップページに戻って右上の「ドメインNavi ログイン」をクリックしてください。（@<img>{onamaeTopPageAgain}）「ドメインNavi」とはお名前.comの管理画面のことです。ログイン画面（@<img>{domainNaviLogin}）が表示されたら、先ほど発行されたばかりのお名前ID@<fn>{forgetOnamaeId}とパスワードを入力して「ログイン」を押してドメインNaviにログインします。@<fn>{reCAPTCHA}

//footnote[forgetOnamaeId][もしお名前IDをメモし忘れてしまったらドメイン登録したときに届くメールにも記載されています]
//footnote[reCAPTCHA][「ログイン」を押すと、操作しているのが本当に人間なのかを確認するために「信号機のタイルをすべて選択してください」のようなreCAPTCHAの画面が表示されることがあります。筆者はよく失敗します。頑張って信号機のタイルだけを選んでください]
...
//footnote[abTest][お名前.comでは定期的にABテストを行っているのか、ログインするお名前IDによってドメインNaviの見た目が大きく異なることがあります。人によって本著とお手元で見ている画面の見た目が一致しないかも知れません。ですが見た目にかかわらず、やりたいことは「今買ったドメイン名で自動更新をオフにしたい」だけですので、画面が違っていても負けずに先へ進みましょう]

//image[continueContract][ドメインNavi ドメイン契約更新][scale=0.8]{
//}

「ドメイン契約更新」の画面が表示されるのは、ドメインNaviがログインするたびに「来年の更新をしませんか？」と聞いてくる仕様だからです。ついさっきドメイン名を買ったばかりで有効期限は1年先ですので無視して先へ進みましょう。先ほど買ったドメイン名の設定変更がしたいので、上部のメニューで「ドメイン一覧」をクリックしてください。

他のページへ移動しようとすると「このサイトを離れますか？行った変更が保存されない可能性があります。」と警告が出る（@<img>{contractRemind}）のでちょっとドキッとしますが、ドメイン名の有効期限は1年も先ですので今更新しなくてもまったく問題ありません。強い気持ちで「このページから移動する」を押してください。
...
この設定を変更したいので、上部の「ドメイン設定」から「ドメイン自動更新設定」を押します。（@<img>{autoUpdate}）

//image[autoUpdate][「ドメイン自動更新設定」をクリック][scale=0.8]{
//}

ドメイン自動更新設定（@<img>{domainAutoRenew}）の画面で、自動更新をオフにしたいドメイン名にチェックを入れたら「確認画面へ進む」をクリックして自動更新を解除します。

//image[domainAutoRenew][ドメインNavi ドメイン自動更新設定][scale=0.8]{
//}

「自動更新設定を解除されますと、更新忘れによりドメインを失効する恐れがありますので、ご注意ください。」と表示されます（@<img>{domainAutoRenewOffNotice}）が構いません。「規約に同意し、上記内容を申し込む」を押してください。

//image[domainAutoRenewOffNotice][ドメイン自動更新解除の確認][scale=0.8]{
//}

再度「変更手続きを続けますか？」という確認（@<img>{domainAutoRenewOffNoticeAgain}）が表示されますので、「解除する」を押してください。

//image[domainAutoRenewOffNoticeAgain][ドメイン自動更新解除の確認][scale=0.8]{
//}

@<img>{domainAutoRenewOff}のように「ドメイン自動更新解除」が「完了」になったら設定変更は完了です。

//image[domainAutoRenewOff][「ドメイン自動更新解除」が完了になっていたらOK][scale=0.8]{
//}
...
//footnote[fiction][重ねて書いておきますがこのトラブルはフィクションです。登場するA社やCさんなどの人物・団体・名称等はすべて架空であり、実在のものとは関係ありません]

ディレクターCさんがドメイン名を買った2年後に転職して会社を去り、そこからさらに3年後にCさんのクレジットカードの有効期限が来ました。お名前.comからは再三に渡って「与信に失敗して決済できなかったよ！クレジットカード情報を新しくして！更新しないと期限切れちゃうよ！」というメール@<fn>{remindMail}が届きますが、宛先は辞めてしまったCさんのメールアドレスなのでA社の人は誰も気づきません。
...
さて、ここまでドメイン名を「買う」「購入する」のように表現してきましたが、ドメイン名は本当に「買う」ものなのでしょうか？@<fn>{registration}ちょっと立ち止まって考えてみましょう。
...
たとえば「@<code>{startdns.fun}というドメイン名を使いたい！」と思ったら、そのドメイン名を「買う」のではありません。レジストラ（お名前.com）を通して、.funというTLDを管理しているレジストリ（DotSpace, Inc.@<fn>{dotSpaceInc}）に対してドメイン名の登録を申請するのです。登録申請を受け付けたレジストリが、内容を確認した上でレジストリデータベースに情報を登録することでドメイン名の登録は完了します。

ドメイン名を登録することで、私たちはそのドメイン名をレジストリが定めたルールの範囲内で一定期間自由に使う権利を得ます。@<fn>{authority}登録したドメイン名には有効期限がありますが、期限が切れる前に更新の手続きをすれば引き続き使用できます。有効期間は基本的に1年単位です。細かな規定はレジストラやTLDにもよりますが、複数年の登録もできますので、長く使うことが先に分かっているドメイン名であれば、最初に登録するタイミングで「10年の登録」をしておくことも可能@<fn>{10year}です。
...
//footnote[registration][ドメイン名の登録 - JPNIC @<href>{https://www.nic.ad.jp/ja/dom/registration.html}]
//footnote[dotSpaceInc][@<href>{https://radix.website/}]
//footnote[authority][より正確に言うと、「委任」という形でそのドメイン名の管理権限を任されます。委任については@<chapref>{whatIsDns}で詳しく説明します]
//footnote[10year][複数年の登録をしていたのにレジストラが倒産してしまった、という場合のリスクについては後述します]

== Whoisとは

ところで先ほどドメイン名を購入するとき「Whois情報公開代行」と表示されていることを確認しました。もし「Whois情報公開代行」のオプションを付けずにドメイン名を購入した場合、「Whois情報を登録してください」という住所や氏名を入力するページが出てきます。
...
まとめるとWhoisとは、そのドメイン名を所有している組織や担当者の氏名、連絡先（住所・電話番号・メールアドレス）、ドメイン名の有効期限などがインターネットで誰でも見られるサービスのことでした。そして前述のとおりWhois情報は基本的に「正確に登録しなければいけない」もの@<fn>{whyWhois}です。
...
クライアントであるA社にそうした技術的な問い合わせや連絡が来ても困ってしまう…という場合は社名や担当者名などはA社にしておいて、メールアドレスはメーリングリストをA社に作ってもらってB社やC社をメンバーに入れるのが得策でしょう。

=== プライバシーを守るためのWhois情報公開代行

仕事の場合は前述のようにWhoisに正しい情報を登録すべきですが、個人でドメイン名を買ったとき自宅の住所や名前をWhoisに載せるのは抵抗があります。そこで「プライバシーを守るため個人情報をWhoisに載せたくない」という人のためにあるのが、先ほど出てきたお名前.comの「Whois情報公開代行」というオプションサービスです。
...
その代わりドメイン名を買った人がレジストラを通してJPRSに「登録者名を非表示にして欲しい」という申請@<fn>{whoisConcealment}を出せば、JPRS側でWhoisの登録者名を「登録者からの申請により非表示」のようにしてくれます。しかし残念ながらお名前.comではドメイン名を買った人からJPRSに対する「登録者名非表示の申請代行」を受け付けていません。そのため結論としてはお名前.comで.jpのドメイン名を買った場合、Whoisの登録者名に自分の氏名が出てしまうのは回避できない、という状況になっています。
...
正解はAです。企業向けの.co.jpや大学向けの.ac.jpなど、組織の種類ごとに用意されている属性型JPドメイン名は基本的に1つの組織につき1つしか登録できません。ただしこの「1組織1ドメイン名のみ」という原則は以前は絶対でしたが現在は緩和されています。2014年に「属性型（組織種別型）・地域型JPドメイン名登録等に関する規則」が改訂され、企業の合併・組織名変更・事業譲渡などに限って、JPRSに「1組織1ドメイン名制限緩和申請」という申請@<fn>{domainRestriction}を出せば1つの組織で複数のco.jpドメイン名を持てるようになったのです。@<fn>{notOnamae}ですが残念ながら今回のケースは単なる社長の思いつきなのでこの制限緩和には当てはまりません。

//footnote[domainRestriction][@<href>{https://jpdirect.jp/organizational/domain-restriction-relaxation.html}]
//footnote[notOnamae][この「1組織1ドメイン名制限緩和申請」はレジストラを通してJPRSに申請を行うのですが、レジストラによって申請代行をサービスとして行っているところと、行っていないところがあります。たとえばお名前.comでは「1組織1ドメイン名制限緩和申請」は受け付けていないため、どうしても緩和申請がしたい場合は、別のレジストラへ移管した上で申請することになります]
...
どうやら2013年7月にワーナーマイカルシネマズがイオンシネマに統合された後、旧ドメイン名@<code>{warnermycal.com}を手放したら、まんまと業者にドロップキャッチされてしまったようです。ドメイン名を手に入れた業者が「あなたのコンピュータでウイルスが検出されました」系の詐欺サイトを開設し、これはあくまで推測ですが「ワーナーマイカルシネマズのサイトを見ようとしたエンドユーザ」が詐欺サイトへアクセスして何かしらの被害が出てしまったので、注意喚起としてこのお知らせを出したようです。
