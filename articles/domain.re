...
このトラブルは信用できるレジストリのTLDを選ぶ以外に回避策がありません。絶対に落とせないサイトやメールを消失させたくないメールアドレスなどで、.ioのようにトラブルを起こしたことのあるTLDを採用するのはあまりお勧めしません。ドメイン名を買うときは、このTLDはどこのレジストリが管理しているのだろう？過去に障害は発生していないか？などを事前に確認しておきましょう。たとえばjpドメインのJPRSやtokyoドメインのGMOインターネットなら、レジストリとしての実績があるため安心です。
...
再度「変更手続きを続けますか？」という確認（@<img>{domainAutoRenewOffNoticeAgain}）が表示されますが、「解除する」を押してください。
...
正解はAです。企業向けの.co.jpや大学向けの.ac.jpなど、組織の種類ごとに用意されている属性型JPドメイン名は基本的に1つの組織につき1つしか登録できません。ただしこの「1組織1ドメイン名のみ」という原則は以前は絶対でしたが現在は緩和されています。2014年に「属性型（組織種別型）・地域型JPドメイン名登録等に関する規則」が改訂され、企業の合併・組織名変更・事業譲渡などに限って、JPRSに「1組織1ドメイン名制限緩和申請」という申請@<fn>{domainRestriction}を出せば1つの組織で複数のco.jpドメインを持てるようになったのです。@<fn>{notOnamae}ですが残念ながら今回のケースは単なる社長の思いつきなのでこの制限緩和には当てはまりません。

//footnote[domainRestriction][@<href>{https://jpdirect.jp/organizational/domain-restriction-relaxation.html}]
//footnote[notOnamae][この「1組織1ドメイン名制限緩和申請」はレジストラを通してJPRSに申請を行うのですが、レジストラによって申請代行をサービスとして行っているところと、行っていないところがあります。たとえばお名前.comでは「1組織1ドメイン名制限緩和申請」は受け付けていないため、どうしても緩和申請がしたい場合は、別のレジストラで移管した上で申請することになります]
