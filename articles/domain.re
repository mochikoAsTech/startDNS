...
他のページへ移動しようとすると「このページから移動しますか？入力したデータは保存されません。」と警告が出る（@<img>{contractRemind}）のでちょっとドキッとしますが、ドメイン名の有効期限は1年も先ですので今更新しなくてもまったく問題ありません。強い気持ちで「このページから移動する」を押してください。
