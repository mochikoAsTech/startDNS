...
また「うちのサイトはwwwありとなしのどちらでアクセスしても同じ画面が出るよ！リダイレクトはされないけどね」という場合もやはり問題です。検索エンジンはFQDNが異なるサイトをまったく別のサイトとして認識するため、wwwありのサイトとなしのサイトでドメイン名としての価値が分散されてしまい、その結果として検索時に上位表示されにくくなります。
...
そしてGoogleが「Chromeで.devを開くと強制的にHTTPSにリダイレクトする」という暴挙に出たのは「Googleは2014年にdevのレジストリ（生産元）になった。でもまだ2017年12月の時点では.devのドメイン名は一般発売していない。ということはその時点で.devを使っている人は全員、ドメイン名買わずに勝手に使ってる人なので迷惑が掛かっても知ったことではない。だから.devはHTTPSに強制リダイレクトさせるね」ということだったのだと推察しています。その後、2019年2月に.devのドメイン名は登録が開始されました。
...
自社サイト（example.net）は今までWeb制作会社のA社にお任せしていたが、サイトリニューアルを機に運用をA社からB社に移管することになった。でも移管と言っても何をすればいいのかふんわりしていてよく分からない…。そんなときは以下のような移管前後の表（@<table>{domainTransfer}）を作って埋めていきましょう。
...
@<table>{domainTransfer}のようにドメイン名の管理はA社からB社に変わるけれど、レジストラはお名前.comのまま変わらないのであれば、お名前.comの「お名前ID付け替え」という機能でA社のお名前IDからB社のお名前IDにドメイン名を移動することができます。
...
ドメイン名の管理やネームサーバの変更は4月上旬に済ませたし、EC2上で動いている新サイトもテストはばっちりです。5月15日のサイトリニューアル当日、B社のCさんはRoute53でexample.netのAレコードの値を203.0.113.111から203.0.113.222に書き換えた後、手元のスマホでサイトを開いてリニューアル後のページが正しく表示されることを確認してからクライアントに「リリース完了」の電話をしました。ですが電話の向こうのクライアントは「え、古いサイトが表示されるんですけど…？」と困惑気味です。
...
キャッシュが消えるまでの時間を逆算して考えると、そもそもCさんはリニューアル日である5月15日の8日以上前にexample.netのAレコードのTTLを300程度まで短くしておいて、当日IPアドレスを書き換えたらすぐに反映されるようにしておくべきでした。
...
事前にTTLを確認した上で短くしておくことで、いわゆる「浸透」を無駄に待たなくて済みますし、TTLを過ぎても新サイトに切り替わらなければ何か他に問題があるのでは？と気づくことができます。サイトリニューアル時は変更対象のリソースレコードのTTLを事前に確認しておきましょう。
...
実際に自分のドメイン名で試してみましょう。筆者はstartdns.funを使ってみますので、あなたもお名前.comで買ったドメイン名を使って試してみてください。

先ずはリソースレコードが存在しないtest.startdns.funをdigで引いてみましょう。statusが「そのドメイン名のリソースレコードは存在しない」という意味のNXDOMAINになって返ってきます。該当するレコードが存在しないためANSWER SECTIONは存在せず、代わりにドメインの管理情報を表すSOAレコードが付加情報として返ってきました。
...
新たにサイトをオープンするとき、まだリソースレコードが用意できていないのにブラウザでサイトを開いてしまい、リソースレコードが用意できた後で再度確認しようとしたらなぜか見られない…という現象はこのネガティブキャッシュが原因です。「インフラ担当に頼んでおいたAレコード、もう用意できたかな？」と確認するときは、フルリゾルバに問い合わせてネガティブキャッシュが残ってしまうことのないよう、@<code>{@}で対象ネームサーバを指定して直接問い合わせる@<fn>{norec}方法がお勧めです。

//cmd{
1. 先ずは自分のドメインのネームサーバを確認する
...
たとえばお名前.comでは、ドメイン名の移管申請の通知を受けると、当該ドメイン名でWhois情報の管理担当者に登録されているメールアドレスに対して「【重要】トランスファー申請に関する確認のご連絡」というメールを送り、移管の意志を確認します。そしてメールに記載された期日までに承認あるいは拒否の手続きが行われなければ、自動的に承認しないものとみなします。

ここは完全に推測ですが、株式会社サンライズが@<code>{lovelive-anime.jp}を登録していたレジストラのA社は、この「期限までに回答がなかった場合」に何もしない、あるいはレジストリであるJPRSと同様に「承認する」とみなした動きをする実装になっていたと思われます。同様の事象発生を防ぐために、あなたも普段利用しているレジストラが、移管の通知を受けたときにどのような動きをするのか確認しておきましょう。

=== 移管できないようロックしておけば安心？

そもそもJPRSから通知を受けたレジストラのA社が、ドメイン名の登録者（株式会社サンライズ）に対して移管を承認するか拒否するかの確認メールを出した際に、登録者がきちんと「移管しない」と回答していればこの乗っ取りは防げていました。しかし重要なメールや通知をうっかり見落としてしまう失敗は、誰しも一度は経験があると思います。勝手に移管が進んでしまわないよう、ロックしておくことはできないのでしょうか？
