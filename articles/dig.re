...
お名前.comのネームサーバでこのようなレコードを作ろうとすると「ホスト名が同じ文字列のレコードを複数ご登録頂く場合は、異なるTTLは設定できません」と表示されて、そもそも設定できませんでした。蛇足ですが筆者が個人で運用しているネームサーバ@<fn>{bindVersion}で前述のような異なるTTLを設定してみた上でdigで問い合わせてみたところ、次のようにTTLは300に揃えられた状態で応答が返ってきました。
...
CDNを使う場合だけでなく、1台のウェブサーバに大量のサイトが相乗りしているような場合もCNAMEレコードを使うと便利です。たとえばウェブサーバにサイトA、サイトB、サイトC…と計100サイトが相乗りしていてそれぞれがAレコードを設定していた場合、ウェブサーバを引っ越すとなったら100件のAレコードを書き換えなければなりません。ですが、Aレコードを設定しているのはサイトAのみで、それ以外のサイトはCNAMEでサイトAのドメイン名を指定するという方法にしておけば、サーバ引っ越しに際して書き換えなければならないのはサイトAのAレコードのみです。
